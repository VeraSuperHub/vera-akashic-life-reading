...
The optional Western/traditional/Vedic astrology helper uses Swiss Ephemeris through `pyswisseph` when installed by the user.
...
- Role: deterministic tropical and sidereal ephemeris backend for planets, houses, ascendant, ayanamsa, nakshatra, and dasha-derived timing context. The traditional Western layer reuses tropical positions and adds local calculations for whole-sign houses, sect, traditional rulership, lots, and annual profections; it adds no new upstream dependency.
