...
The bundled helper script uses `lunar_python` by 6tail for deterministic 八字 calculation.

- Repository: https://github.com/6tail/lunar-python
- License: MIT
- Role: deterministic Chinese solar/lunar calendar and 八字 calculation backend.
...
The bundled 紫微斗数 helper uses `iztro` by SylarLong.
...
The optional 西占/古占/印占 helper uses Swiss Ephemeris through `pyswisseph` when installed by the user.
...
- Role: deterministic tropical and sidereal ephemeris backend for planets, houses, ascendant, ayanamsa, nakshatra, and dasha-derived timing context. The 古占 layer reuses tropical positions and adds local calculations for whole-sign houses, sect, traditional rulership, lots, and annual profections; it adds no new upstream dependency.
